...
chown -R new_user:new_user /etc/ssh/new_user

chmod 755 /etc/ssh/new_user

chmod 644 /etc/ssh/new_user/authorized_keys

sed -i -e '/^#AuthorizedKeysFile/s/^.*$/AuthorizedKeysFile \/etc\/ssh\/new_user\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "AllowUsers new_user" >> /etc/ssh/sshd_config'
...
sed -i -e '/^Port/s/^.*$/Port 1103/' /etc/ssh/sshd_config

firewall-cmd --add-port 1103/tcp --permanent
...
sh -c 'echo "destemail = kwin.2011@gmail.com" >> /etc/fail2ban/jail.local'
...
cat /home/new_user/.credentials | chpasswd

rm /home/new_user/.credentials